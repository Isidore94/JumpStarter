...
- No third-party dependency in `tools/`. It runs on a bare Python 3.9+.
...
|---:|---|---|
| **0 — NOW** | First real retrofit | Run the audit against a real repo and fix what it gets wrong |
| **1 — NEXT** | Close the loop on the templates | Every template proven by being used once |
...
1. **Bootstrap one real new project from the templates.** — `PLANNED`. Every placeholder
   ...
   answer and which placeholders had no good answer. Gate 2.
...
1. the audit has run against at least one real existing repository and its false
   positives are fixed or documented;
